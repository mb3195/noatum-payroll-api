...
# IMPORTANT: when using "*" you MUST set allow_credentials=False
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],      # later you can lock this to the Blocks origin
...
def is_bird_day(s):
    s = norm_sched(s)
    return ("eagles/hawks" in s or "eagleshawks" in s or
            "08:00 - 20:00" in s or "08:00-20:00" in s or "8am-8pm" in s)

def is_am_day(s):
    s = norm_sched(s)
    return ("06:00 - 14:00" in s or "06:00-14:00" in s or
            "6am-2pm" in s or "am 6-2" in s)

def is_pm_day(s):
    s = norm_sched(s)
    return ("14:00 - 22:00" in s or "14:00-22:00" in s or
            "2pm-10pm" in s or "pm 2-10" in s)

def is_bird_night(s):
    s = norm_sched(s)
    return ("20:00 - 08:00" in s or "20:00-08:00" in s or
            "8pm-8am" in s or "owls/falcons" in s)

def is_std_night(s):
    s = norm_sched(s)
    if is_bird_night(s):
        return False
    return ("20:00-06:00" in s or "22:00-06:00" in s or
            "8pm-6am" in s or "10pm-6am" in s or "nightshift22:00-06:00" in s)

def is_22_06_night(s):
    s = norm_sched(s)
    return ("22:00-06:00" in s or "22:00 - 06:00" in s or
            "nightshift22:00-06:00" in s)
    ...
    for s, e in segments:
        ...
        while cur < e:
            ...
            if AM_START <= local < AM_END:
                ...
                key = "NIGHT"
                boundary = cur - local + (AM_START if local < AM_START else 1440 + AM_START)
            nxt = min(boundary, e)
            mins[key] += nxt - cur
            cur = nxt
    total = sum(mins.values())
    return mins["AM"]/60.0, mins["PM"]/60.0, mins["NIGHT"]/60.0, total/60.0
...
def distribute_day(day_row, daily_total, schedule_str):
    ...
    if is_am_day(s) or is_pm_day(s):
        ...
        return (tot, 0.0, 0.0, shift_type) if shift_type == "am_day" else (0.0, tot, 0.0, shift_type)
...
def process_timesheet(df):
    ...
    while i < len(df):
        ...
        for dr in daily_rows:
            ...
            try:
                ...
                # skip bad date formats instead of crashing whole run
                continue
    ...
    cols = [
        "CANDIDATE NAME", "TOTAL HOURS", "AM HOURS", "PM HOURS",
        "AM OVERTIME", "PM OVERTIME", "NIGHTS HOURS", "NIGHTS OVERTIME",
        "TOTAL OVERTIME", "FLAG WITH POTENTIAL ISSUE", "BIRD SHIFT", "HOLIDAYS",
    ]
