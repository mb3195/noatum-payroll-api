...
# IMPORTANT: allow_credentials must be False when using "*"
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],      # later you can restrict to the Blocks origin
...
class ProcessRequest(BaseModel):
    # Preferred: send csvBase64 from Blocks
    csvBase64: str | None = None
    # Fallback: publicly reachable URL (won't work with private Blocks URLs)
...
async def process_payroll(request: ProcessRequest):
    try:
        # 1) Get CSV bytes from base64 or URL
